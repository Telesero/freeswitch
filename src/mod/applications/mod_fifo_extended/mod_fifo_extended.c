 ...
 *
 * mod_fifo_extended.c -- FIFO
 *
 */
#include <switch.h>
#define FIFO_APP_KEY "mod_fifo_extended"

SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_fifo_extended_shutdown);
SWITCH_MODULE_LOAD_FUNCTION(mod_fifo_extended_load);
SWITCH_MODULE_DEFINITION(mod_fifo_extended, mod_fifo_extended_load, mod_fifo_extended_shutdown, NULL);
...
static switch_status_t consumer_read_frame_callback(switch_core_session_t *session, switch_frame_t *frame, void *user_data)
{
	...
	switch_channel_t *channel = switch_core_session_get_channel(session);
	const char *bridge_uuid_wait = switch_channel_get_variable(channel, "fifo_bridge_uuid_wait");

	if (switch_true(bridge_uuid_wait)) {
		const char *bridge_uuid = switch_channel_get_variable(channel, "fifo_bridge_uuid");

		if (strcasecmp(bridge_uuid, "undef")) {
			return SWITCH_STATUS_BREAK;
		}
		return SWITCH_STATUS_SUCCESS;
	}
	...
	case SWITCH_MESSAGE_INDICATE_UNBRIDGE:
		if (msg->numeric_arg == 42) {
			...
			/* When a session is interrupted to execute an application
			   ...
			   jitterbuffer) but we need mod_fifo_extended not to see the
...
static uint32_t fifo_add_outbound(const char *node_name, const char *url, uint32_t priority, const char *uuid, const char *record_template)
{
	fifo_node_t *node;
	...
	if (record_template) {
		switch_event_add_header_string(call_event, SWITCH_STACK_BOTTOM, "dial-record-template", record_template);
	}
...
// fifo_add_outbound <node> <url>[ <call_uuid>][record_template][ <priority>]
SWITCH_STANDARD_API(fifo_add_outbound_function)
{
	char *data = NULL, *argv[6] = { 0 };
	int argc;
	uint32_t priority = 0;
	...
	stream->write_function(stream, "%d", fifo_add_outbound(argv[0], argv[1], priority, argv[2], argv[3]));
...
SWITCH_STANDARD_APP(fifo_function)
{
	...
	if (!consumer) {
		...
		while (switch_channel_ready(channel)) {
			...
			if (moh) {
				...
				if (!SWITCH_READ_ACCEPTABLE(rstatus)) { // serviceability improvement: plays default moh in case if http fetch fails
					rstatus = switch_ivr_play_file(session, NULL, "queue-moh.wav", &args);
				}
			} else {
	...
	abort:
		...
		const char *check_wait = switch_channel_get_variable(channel, "fifo_bridge_uuid_wait");
		...
		while (switch_channel_ready(channel)) {
			...
			const char *url_record_template = NULL;
			...
			*/
			...
			url_record_template = switch_core_session_strdup(session, switch_event_get_header(pop, "dial-record-template"));
			...
			if (url) {
				...
				if (switch_ivr_originate(session, &other_session, &cause, url, 120, NULL, NULL, NULL, NULL, NULL, SOF_NONE, NULL, NULL) != SWITCH_STATUS_SUCCESS) {
					...
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
									  "Fifo Originate to [%s] success, dial uuid=[%s], caller uuid=[%s], record template=[%s]\n", url,
									url_caller_uuid, caller_uuid, url_record_template);
...
SWITCH_MODULE_LOAD_FUNCTION(mod_fifo_extended_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	SWITCH_ADD_API(commands_api_interface, "fifo_add_outbound", "Add outbound members to a fifo. Use call_uuid to track if the right call is connected. This is also required when used with fifo_bridge_uuid_wait.", fifo_add_outbound_function, "<node> <url> [<call_uuid>] [record_template] [<priority>]");
